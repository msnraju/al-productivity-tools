...
export class ObjectWriter {
  static write(context: IObjectContext): string {
    ...
    if (context.fields) {
      const fieldLines = FieldsWriter.write(context.fields);
      fieldLines.forEach((line) => lines.push(line));
      lines.push("");
    }

    if (context.layout) {
      const layoutLines = LayoutWriter.write(context.layout);
      layoutLines.forEach((line) => lines.push(line));
      lines.push("");
    }

    if (context.actions) {
      const actionLines = ActionContainerWriter.write(context.actions, 4);
      actionLines.forEach((line) => lines.push(line));
      lines.push("");
    }

    if (context.views) {
      const viewLines = ViewContainerWriter.write(context.views, 4);
      viewLines.forEach((line) => lines.push(line));
      lines.push("");
    }

    if (context.dataSet) {
      const dataSetLines = DataSetWriter.write(context.dataSet);
      dataSetLines.forEach((line) => lines.push(line));
      lines.push("");
    }

    if (context.schema) {
      const schemaLines = SchemaWriter.write(context.schema);
    ...
    if (context.triggers.length > 0) {
      context.triggers.forEach((trigger) => {
        const triggerLines = FunctionWriter.write(trigger, 4);
    ...
    if (context.procedures.length > 0) {
      ...
      procedures.forEach((procedure) => {
        const procedureLines = FunctionWriter.write(procedure, 4);
